...
2. Open **SQL Editor** and run the migrations in `supabase/migrations/` in order:
   ...
   - [`0005_acknowledged_gap.sql`](./supabase/migrations/0005_acknowledged_gap.sql)
     — adds `subcontractors.acknowledged_gap`, the receipt-vs-entered difference
     already waved through, so the row stops flagging that one.
...
The app installs to a home screen and runs without browser chrome, which is what
it is for — the person using it is standing on a slab with one hand free.

- **Manifest** — [`public/manifest.json`](./public/manifest.json). Standalone
  display, kraft `#EDE6D6` for both the splash background and the theme colour,
  so the status bar and the page beneath it are the same surface. Icons are
  `/icon-192.png` and `/icon-512.png`.
- **iOS** — Safari reads none of the manifest for home-screen installs, so the
  root layout emits its own tags: `apple-mobile-web-app-capable` (both the
  standard spelling and the pre-iOS-17.4 one), the app title, a `default`
  (light) status bar to match the kraft, and a 180×180 `apple-touch-icon`.
- **Service worker** — generated by `next-pwa` into `public/sw.js` at build
  time, registered by [`components/service-worker.tsx`](./components/service-worker.tsx).

### What the worker holds

The precache is the build's own output — around 60 files: every JS chunk, the
stylesheet, the self-hosted fonts, the icons and the manifest. On top of that,
three runtime rules: `/_next/static/*` and fonts cache-first (the bytes behind a
hashed URL never change), images stale-while-revalidate.

Nothing else. Documents, RSC payloads, `/api/*` and every Supabase call go
straight to the network with the worker standing aside — including `/`, which
`cacheStartUrl: false` and `dynamicStartUrl: false` keep out between them. Those
carry live money figures from behind a login, and a stale total served
confidently from a cache is worse than a page admitting it cannot reach the
server. So the shell survives a lost signal; the ledger does not, on purpose.

The worker and the manifest are exempted from the auth gate in
[`proxy.ts`](./proxy.ts). A service worker answering with a redirect to the login
page does not register at all.

### The webpack build

`next-pwa` is a webpack plugin, and Next 16 builds with Turbopack, which refuses
to run alongside a webpack config rather than pretending otherwise. So
`npm run build` passes `--webpack`.

`npm run dev` still runs Turbopack, because the plugin is only applied when
`NODE_ENV` is `production` — its own `disable` option is checked *inside* the
webpack hook, so the hook gets attached either way and Turbopack still refuses.
Nothing is cached in development, which is what you want during a reload.

If the webpack build ever costs more than it is worth, [Serwist][serwist] is the
maintained successor Next's own PWA guide points to, it runs under Turbopack, and
the Workbox config above ports across nearly unchanged.
...
- No push notifications, no camera-only capture flow, no offline write queue.
  Attaching a file offers Scan (the camera on a phone) and Attach, and both need
  a connection to upload.
